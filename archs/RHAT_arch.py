...
from .MFHAT_arch import MFHAT


@ARCH_REGISTRY.register()
class BasicRecurrentHAT(nn.Module):
    ...
    def propagate(self, feats, flows, module_name):
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i == 0:
                ...
                if module_name in self.patch_align:
                    
                    feat_prop = self.patch_align[module_name](cond)
                else:
                    print(self.patch_align.keys())
                    raise KeyError(f"Module '{module_name}' not found in 'self.patch_align'. This may due to torch.cuda is inactive")
...
if __name__ == '__main__':
    ...
    model = BasicRecurrentHAT(
        ...
        cpu_cache_length = 100,
        is_low_res_input = True,
        spynet_path = r'Github_proj\RethinkVSRAlignment\experiments\pretrained_models\flownet\spynet_sintel_final-3d2a1287.pth'
    )

    print(model)
    print(f"flops {model.flops() / 1e9} 'G'")
